...
router.post('/upgrade', auth, checkPerfil('Admin'), (req, res) => {
  ...
  db.run(
    'UPDATE empresas SET plano_ativo = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', 
    [plano, empresaId], 
    (err) => {
      if (err) {
...
router.get('/status', auth, (req, res) => {
  ...
  db.get(
    ...
    (err, result) => {
      ...
      // Verificar trial
      db.get(
        'SELECT data_inicio_trial, data_fim_trial FROM empresas WHERE id = ?',
        [empresaId],
        (err, empresa) => {
          if (err) {
            return res.status(500).json({ erro: 'Erro ao buscar dados da empresa' });
          }
          ...
          res.json({
            plano: planoInfo,
            uso: {
              calculosUsados: result.count,
              limite: planoInfo.calculos === -1 ? 'ilimitado' : planoInfo.calculos,
              percentual: planoInfo.calculos === -1 ? 0 : ((result.count / planoInfo.calculos) * 100).toFixed(1)
            },
            trial: infoTrial
          });
        }
